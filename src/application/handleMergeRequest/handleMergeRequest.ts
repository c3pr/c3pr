...
export default function handleMergeRequest(webhookPayload: GitLabMergeRequestUpdated, {lcid, euuid}) {
    ...
    if (webhookPayload.object_attributes.author_id !== config.c3pr.repoGitlab.gitlab.bot_user_id) {
        c3prLOG4(`MR event is ignored. The PR is not authored by the bot.`, {lcid, euuid, meta: {webhookPayload, bot_user_id: config.c3pr.repoGitlab.gitlab.bot_user_id}});
        return;
    }
    if (webhookPayload.object_attributes.action === 'open' && webhookPayload.changes.state === undefined) {
        c3prLOG4(`MR 'open' event is ignored. The PR will be first created in the database by the PullRequestCreated event.`, {lcid, euuid, meta: {webhookPayload}});
        return;
    }

    return createAndEmitPullRequestUpdated(webhookPayload, {lcid, euuid});
}