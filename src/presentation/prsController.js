...
module.exports = function (app) {
    ...
    app.post('/prs', function (request, response) {
        ...
        if (!prs.meta ||
            ...
            c3prLOG(errorMessage, {prs}, {nodeName: 'c3pr-repo-gitlab', correlationIds: [prs.meta && prs.meta.correlationId], moduleName: 'prsController'});
            response.status(400).send(errorMessage);
            return;
        }
        c3prLOG(`pr received. Title: ${prs.patch.title} - Repo: ${prs.repository.url}`, {prs}, {nodeName: 'c3pr-repo-gitlab', correlationId: prs.meta.correlationId, moduleName: 'prsController'});
        handlePrs(prs);
