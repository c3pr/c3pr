...
#CMD replace-in-file /__C3PR_AGENT_EXECUTABLE_VERSION__/g ${AGENT_VERSION} c3pr-agent.js --isRegex && \
#    pkg --debug --targets node8-alpine-x64 --output /release/c3pr-agent-alpine-${AGENT_VERSION} c3pr-agent.js
CMD replace-in-file /__C3PR_AGENT_EXECUTABLE_VERSION__/g ${AGENT_VERSION} c3pr-agent-executable.js --isRegex && \
