...
async function applyGitPatchBase64(cloneFolder: string, gitUserName: string, gitUserEmail: string, patch: PatchDiffBase64, {ids}) {
    ...
    await c3prSH3(`git -c user.name='${userNameNoQuotes}' -c user.email='${userEmailNoQuotes}' am --keep-cr --whitespace=fix ${patchFileName}`, {cwd: cloneFolder}, {ids});
