...
async function handleFirstCollectedEvent({event_type, handlerFunction, c3prHubUrl, jwt, lcid: outerLCID, sha: outerSHA, euuid: outerEUUID}) {
    ...
    c3prLOG4(`Handling ${event_type}.`, {lcid, sha, euuid});
    ...
    try {
        ...
        if (!event) {
            c3prLOG4(`No ${event_type} collected (possibly due to concurrent attempts to collect the same event and this instance was late). Skipping.`, {lcid, sha, euuid, meta: {event}});
            return;
        }
    } catch (error) {
        c3prLOG4(`Couldn't collect ${event_type}. Skipping.`, {lcid, sha, euuid, error});
        return;
    }
    euuid = event.uuid;
    ...
    try {
        ...
        c3prLOG4(`Error while executing handlerFunction() for event handling.`, {lcid, sha, euuid, error, meta: {handlerFunction, event}});
        ...
        markAsUnprocessed({event_type, uuid: event.uuid, c3prHubUrl, jwt, lcid, sha, euuid}).catch(error => {
            c3prLOG4(
                ...
                {lcid, sha, euuid, error, meta: {handlerFunction, event}}
            );
        });
        return;
    }
